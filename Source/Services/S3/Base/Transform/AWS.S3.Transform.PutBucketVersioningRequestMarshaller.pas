...
begin
  ...
  try
    ...
    try
      if PublicRequest.VersioningConfig <> nil then
      begin
        XmlWriter.WriteStartElement('VersioningConfiguration', '');
        if PublicRequest.VersioningConfig.IsSetMFADelete then
          XmlWriter.WriteElementString('MfaDelete', '', PublicRequest.VersioningConfig.MFADelete.Value);
        if PublicRequest.VersioningConfig.IsSetStatus then
          XmlWriter.WriteElementString('Status', '', PublicRequest.VersioningConfig.Status.Value);
